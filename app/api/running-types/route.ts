...
export async function GET() {
  ...
  const { data, error } = await supabase.from('running_types').select('*,school:schools(*)').order('name');
  if (error) return NextResponse.json({ error: '加载跑步类型失败' }, { status: 500 });
  return NextResponse.json({ running_types: deduplicateConfigurationsByName(data || [], 'school_id') });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    const existing = await findConfigurationByName(supabase, 'running_types', parsed.data.name);
    if (existing) return NextResponse.json({ running_type: existing });
    const { data, error } = await supabase.from('running_types').insert({ ...parsed.data, school_id: null }).select().single();

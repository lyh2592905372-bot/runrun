...
export async function readProgressRecords(supabase: SupabaseClient, accountIds: string[], userId?: string): Promise<ProgressRun[]> {
  ...
  for (let index = 0; index < accountIds.length; index += 100) {
    ...
    for (let from = 0; ; from += 500) {
      let query = supabase.from('sport_world_run_records')
...
export async function refreshOrderProgressAfterSync(supabase: SupabaseClient, accountId: string) {
  ...
  const { data: account, error } = await supabase.from('accounts')
    .select('id,user_id,order_time,order_count,distance_per_run').eq('id', accountId).is('deleted_at', null).single();
